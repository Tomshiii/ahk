;// this is a version of the `tool` lib that attempted to be able to stack tooltips
;// got quite close, but couldn't get it to take into account the height of the previous tooltip and stack below it
 ...
 ***********************************************************************/
 ...
 class tool {
     ...
     */
    static Cust(message, timeout := 1000, find := false, x?, y?, WhichToolTip?, cascade := 0)
    {
        ;// doing some setup
        ...
        if IsSet(WhichToolTip) ;doing some checks for the whichtooltip variable
            ...
            {
                var := WinGetList("ahk_class tooltips_class32")
                num := 1
                hwnd := ""
                width := unset
                height := unset
                ...
                WhichToolTip := num
                offset := num > 1 ? 25 : 0
                if cascade = 1 && both != true
                    y := ((num*offset)+height+y)-offset
                FileAppend(
                    "msg: " message "`n"
                    "num: " num "`n"
                    "offset: " offset "`n"
