        ...
        for k, v in this.maps {
            ...
            for name, val in v.OwnProps() {
                ...
                for k2, v2 in val {
                    ...
                    if (!InStr(adobecmd, "|||") && (StrLen(adobecmd) + StrLen(Format(' && mklink /D "{1}\{4}\{2}" "{1}\{4}\{3}"', imgsrchPath, k2, v2, which))) >= 8100) ||
                        ((StrLen(Format('{1} && mklink /D "{2}\{5}\{3}" "{2}\{5}\{4}"', imgsrchPath, k2, v2, which)) + StrLen(SubStr(adobecmd, InStr(adobecmd, "|||",,, -1)))) >= 8100) {
